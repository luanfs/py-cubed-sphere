...
####################################################################################
def div_sphere(cs_grid, ll_grid, simulation, map_projection, plot):
    N  = cs_grid.N       # Number of cells in x direction
    ic = simulation.ic   # Funcion to have divergence computed
    ...
    ulon_edx, vlat_edx, ulon_edy, vlat_edy, gQ = init_vars_div(cs_grid, simulation, N, nghost) 
...
def output_div(cs_grid, ll_grid, plot, div_exact, div_numerical, \
    ...
    if plot:
        ...
        #plot_scalar_and_vector_field(div_exact_ll, ulon_edx, vlat_edx, ulon_edy, vlat_edy, 'div_exact_ic'+str(simulation.ic)+"_mono_"+simulation.monot+"_"+simulation.fvmethod, cs_grid, ll_grid, map_projection, colormap, qmin, qmax)
        ...
        qmin = np.amin(div_ll)
        qmax = np.amax(div_ll)
        plot_scalar_and_vector_field(div_ll, ulon_edx, vlat_edx, ulon_edy, vlat_edy, 'div_ic'+str(simulation.ic)+"_mono_"+simulation.monot+"_"+simulation.fvmethod, cs_grid, ll_grid, map_projection, colormap, qmin, qmax)
        ...
        #qmin = -7.6*10**(-5)
        #qmax =  7.6*10**(-5)
        plot_scalar_and_vector_field(div_error_ll, ulon_edx, vlat_edx, ulon_edy, vlat_edy, 'div_error'+'_ic'+str(simulation.ic)+"_mono_"+simulation.monot+"_"+simulation.fvmethod, cs_grid, ll_grid, map_projection, colormap, qmin, qmax)
...
####################################################################################
def init_vars_div(cs_grid, simulation, N, nghost):
    # Interior cells index (ignoring ghost cells)
    i0   = cs_grid.i0
    iend = cs_grid.iend
    j0   = cs_grid.j0
    ...
    # Get velocities
    ulon_edx[:,:,:], vlat_edx[:,:,:] = vector_field(edx_lon, edx_lat, simulation)
    ulon_edy[:,:,:], vlat_edy[:,:,:] = vector_field(edy_lon, edy_lat, simulation)
    ...
    div_exact.f = div(center_lon[i0:iend,j0:jend], center_lat[i0:iend,j0:jend], cs_grid.R, simulation)
...
def error_analysis_div(simulation, map_projection, plot, transformation, showonscreen, gridload):
    ...
    for i in range(0, Ntest):
        ...
        error_linf[i], error_l1[i], error_l2[i] = div_sphere(cs_grid, ll_grid, simulation, map_projection, True)
    ...
    filename = graphdir+"div_tc"+str(tc)+"_ic"+str(ic)+"_cr_rate_"+transformation+"_mono_"+simulation.monot+"_"+simulation.fvmethod
    plot_convergence_rate(Nc, error_linf, error_l1, error_l2, filename, title)

    # Error convergence
    title = "Convergence of error  - divergence operator"
    filename = graphdir+"div_tc"+str(tc)+"_ic"+str(ic)+"_error_convergence_"+transformation+"_mono_"+simulation.monot+"_"+simulation.fvmethod
